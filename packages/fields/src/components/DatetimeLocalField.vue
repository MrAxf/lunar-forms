...
const {
  theme: {
    ...
    classes: {
      ...
      fields: { datetimeLocal: fieldClasses },
    },
  },
  messages,
} = usePluginOptions();
...
const minMaxDateAttrs = computed(() => {
  ...
  const max = new Date(props.max as any);

  return {
    min: isNaN(min as any) ? null : min.toISOString().slice(0, 16),
    max: isNaN(max as any) ? null : max.toISOString().slice(0, 16),
  };
});
</script>
